...
from utils.utils import get_2d_coord_np, crop_resize_by_warp_affine, draw_3d_bbox_with_coordinate_frame
...
def main():
            ...
            for i in range(len(batch['img_name'])):
                ...
                if img_name in img_to_id and category_name in category_to_id:
                    ...
                    gt = {
                        ...
                        "depth": batch["gt_t"][i].cpu().numpy().tolist()[2],
                        "bbox3D": [
                            output_dict["gt_pts_3d"][i][0].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][4].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][6].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][2].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][1].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][5].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][7].cpu().numpy().tolist(),
                            output_dict["gt_pts_3d"][i][3].cpu().numpy().tolist()
                        ],
                    }
                    prediction = {
                        "image_id": img_id,
    ...
    with open("pred_info.json", "w") as f:
