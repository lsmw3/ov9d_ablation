...
def main():
    ...
    # model = model.load_from_checkpoint("logs/objectron_old/epoch=99.ckpt", args=args)
        ...
        for idx, batch in enumerate(tqdm(inference_loader, desc="Inference")):
            ...
            for i in range(len(batch['img_name'])):
                ...
                if img_name in img_to_id and category_name in category_to_id:
                    ...
                    # Image.fromarray(batch['raw_scene'][i].permute(1, 2, 0).cpu().numpy().astype(np.uint8)).save(f"test_pose/scene_{n}.png")
                    # Image.fromarray((batch['image'][i].permute(1, 2, 0).cpu().numpy()*255.).astype(np.uint8)).save(f"test_pose/img_roi_{n}.png")
                    # Image.fromarray((batch['mask'][i].cpu().numpy()*255.).astype(np.uint8)).save(f"test_pose/mask_{n}.png")
                    
                    # scene_img = batch['raw_scene'][i].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                    # gt_pose = draw_3d_bbox_with_coordinate_frame(scene_img, kps_3d[i], batch['gt_r'][i].cpu().numpy(), batch['gt_t'][i].cpu().numpy(), batch['cam'][i].cpu().numpy())
                    # pred_pose = draw_3d_bbox_with_coordinate_frame(scene_img, batch['kps_3d_m'][i].cpu().numpy(), output_dict["pred_ego_rot"][i].cpu().numpy(), output_dict["pred_trans"][i].cpu().numpy(), batch['cam'][i].cpu().numpy())
                    # Image.fromarray(gt_pose).save(f"test_pose/gt_pose_{n}.png")
                    # Image.fromarray(pred_pose).save(f"test_pose/pred_pose_{n}.png")
    ...
    # torch.save(predictions_list, "logs/inference_pth/instances_predictions_3.pth")
