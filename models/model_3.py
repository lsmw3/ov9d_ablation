...
class OV9D(nn.Module):
    def __init__(self, args=None):
        ...
        self.pnp_net = ConvPnPNet(nIn=5, featdim=128, rot_dim=args.rot_dim, num_layers=6)
    ...
    def forward(self, x, mask, mask_latent, c, s, roi_coord_2d):
        ...
        with torch.no_grad():
            ...
            feat_roi = feat_roi * mask_latent.unsqueeze(1)
        ...
        out_nocs_up = F.interpolate(out_nocs, (self.args.scale_size, self.args.scale_size), mode='bicubic', align_corners=True)

        out_nocs_feat = out_nocs_up
        ...
        out_dict = {'pred_nocs_feat': out_nocs, 'pred_nocs_ori_size': out_nocs_up, 'pred_r': pred_rot_m, 'pred_t': pred_t_, 'pred_dims': pred_dims}
...
class Decoder_RT(nn.Module):
        ...
        for i in range(len(conv_dims)):
            ...
            if i == 0:
                self.conv_layers.append(
                    nn.Sequential(
                        nn.Conv2d(last_dim, out_dim, kernel_size=6, stride=4, padding=1),
    ...
    def forward(self, x):
        ...
        x = x.reshape(-1, self.middle_dim * 8 * 8)
