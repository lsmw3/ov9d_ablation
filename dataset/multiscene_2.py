...
class multiscene_2(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        scene_padded, scene_padded_resized = self.pad_and_resize_image(raw_scene)
        ...
        out_dict = {
            ...
            'padded_scene': (scene_padded.transpose((2, 0, 1)) / 255).astype(np.float32), # (3, max(H, W), max(H, W))
            'input_scene': (scene_padded_resized.transpose((2, 0, 1)) / 255).astype(np.float32), # (3, h, w)
    ...
    def pad_and_resize_image(self, image):
        """
        Pad image to square and resize to the largest multiple of 14 that's smaller than the padded size
        ...
        # Find the largest multiple of 14 that's smaller than max_dim
        target_size = (max_dim // 14) * 14
        ...
        target_dimensions = (target_size, target_size)  # Width, height for PIL
        padded_resized_img = Image.fromarray(padded_img).resize(target_dimensions, Image.BILINEAR)
        padded_resized_img = np.array(padded_resized_img)
        
        return padded_img, padded_resized_img
